...
def perform_EDM(index, length, n_replicates, rho, noise, variance, x, y, z):
    ...
    x_ += np.random.normal(0.0, noise, size=len(x_))
    x_, _ = preprocessing(x_, t_, loc=0)
    ...
    for j in range(n_replicates):
        ...
        # preprocessing
        x_, _ = preprocessing(x_, t_, loc=j+1)
        x_ += np.random.normal(0.0, noise, size=len(x_))
        x_, _ = preprocessing(x_, t_, loc=j+1)
    ...
    ts_test = [point for point in collection if (point.time_stamp >= length) & (point.time_stamp < length + 10)]
    del (collection)
...
def partial_function(n_repl, indices, variance, length, noise, rho, x, y, z):
    results = []

    for i in indices:
        partial_result = perform_EDM(i,length, n_repl, rho, noise, variance, x[i-20:i+20], y[i-20:i+20], z[i-20:i+20])
...
def loop(rho, length, noise, var):
    ...
    indices = np.random.randint(20, 1020, size=n_iterations)

    x, y, z, t = simulate_lorenz([1, 1, 1], [10, rho, 8 / 3], 2040, 35.7, 0)
