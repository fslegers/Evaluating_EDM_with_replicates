...
def perform_EDM(ts, initial_point, length, n_replicates, noise, variance, test, rho):
    ...
    if test == "begin_conditions":
        new_x, new_t = sample_multiple_initial_values(initial_point, length-1, n_replicates, noise, variance, rho)
    else:
        new_x, new_t = sample_multiple_rhos(initial_point, length-1, n_replicates, noise, variance, rho)
...
def partial_function(variance, initial_points, n_replicates, length, noise, test, rho):
    ...
    for i in range(len(initial_points)):
        ...
        x += np.random.normal(0.0, noise, len(x))
        x, _ = preprocessing(x, t, loc=0)
