        ...
        for variance, rho in product(df['variance'].unique(), df['Rho'].unique()):
            ...
            for noise in [0.0, 0.05, 0.1]:
                ...
                for length in df_sub_sub['Original_Length'].unique():
                    ...
                    # Change all Nones to 18
                    info = [18 if x is None else x for x in info]
                    ...
                    label = len(sub_df_for_plotting[sub_df_for_plotting['n_replicates'] > 16])
                    #label = label/2.5
                    label = f"{label:.0f}%"
                    labels.append(label)
                    ...
                    sub_df_for_plotting = sub_df_for_plotting[sub_df_for_plotting['n_replicates'] <= 16]
            ...
            axes.set_yticks(list(np.arange(0,17,2)) + [18])
            axes.set_yticklabels([str(i) for i in np.arange(0,17,2)] + ['>16'])
...
def make_violin_plot_variance(df_full):
    plt.rcParams['font.size'] = 20

    for test in df_full['Test'].unique():
        df = df_full[df_full['Test'] == test]

        for noise, rho in product(df['noise'].unique(), df['Rho'].unique()):

            df_sub = df[(df['noise'] == noise) & (df['Test'] == test) & (df['Rho'] == rho)]
            ...
            for variance in [1.0, 5.0, 10.0]:
                df_sub_sub = df_sub[df_sub['variance'] == variance]
                ...
                for length in df_sub_sub['Original_Length'].unique():
                    ...
                    sub_df_for_plotting = pd.DataFrame(info, columns=['n_replicates'])
                    sub_df_for_plotting['length'] = length
                    sub_df_for_plotting['variance'] = variance
            ...
            # density_norm = count: the width of a violin is proportional to the number of observations
            sns.violinplot(data=df_for_plotting, x='variance', y='n_replicates', hue='length', palette='tab10', ax=axes,
                           cut=0, zorder=2, density_norm='count', alpha=.75, legend=False,
                           inner_kws=dict(box_width=12, whis_width=4, color='.2'))

            sns.stripplot(data=df_for_swarmplot, x='variance', y='n_replicates', hue='length', palette='tab10', legend=False,
                          dodge=True, jitter=0.15, size=8, alpha=.4, zorder=1)
            ...
            #plt.title(f'Variance: {variance}, Test: {test}, Rho: {rho}')
            path = 'C:/Users/5605407/Documents/PhD/Chapter_1/Resultaten/length vs replicates/violinplots/'
            plt.savefig(path+f'variance, {noise}, {test}, {rho}.png', dpi=200)
            plt.show()
...
if __name__ == "__main__":
    ...
    make_violin_plot_variance(combined_data)
